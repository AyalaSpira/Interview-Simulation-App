...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer(options =>
    {
        ...
        {
            ValidateIssuer = false, // ✅ שנה ל-false: הטוקן שלך לא מכיל Issuer כרגע
            ValidateAudience = false, // ✅ שנה ל-false: הטוקן שלך לא מכיל Audience כרגע
            ValidateLifetime = true,
            ValidateIssuerSigningKey = true,
            // ValidIssuer = builder.Configuration["Jwt:Issuer"], // ניתן להסיר אם ValidateIssuer=false
            // ValidAudience = builder.Configuration["Jwt:Audience"], // ניתן להסיר אם ValidateAudience=false
            IssuerSigningKey = new SymmetricSecurityKey(
                Encoding.UTF8.GetBytes(builder.Configuration["Jwt:Key"])
            ),
            // === קריטי: זה אומר לשרת איפה לחפש את ה-Role בטוקן ===
            RoleClaimType = ClaimTypes.Role // ✅ הוסף או וודא שזה קיים
        };
    });
// === Controllers ===
